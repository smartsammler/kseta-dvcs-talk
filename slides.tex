...
\begin{frame}{Commit Graphs}
  Since all commits have a parent, they form a \alert{directed graph} without
  cycles (DAG).
...
\begin{frame}{Single User Workflow}
  \begin{enumerate}
    ...
    \item Create a commit
      ...
      \end{enumerate}
  \end{enumerate}
  \includegraphics[width=\linewidth]{pics/init0.png}
\end{frame}
...
\begin{frame}[t]{Single User Workflow}
  ...
  \end{enumerate}
  \begin{overprint}
  \includegraphics<1>[width=\linewidth]{pics/commit0.png}
  \includegraphics<2>[width=\linewidth]{pics/commit1.png}
  \includegraphics<3>[width=\linewidth]{pics/commit2.png}
    \end{overprint}
\end{frame}
...
\begin{frame}[t]{Branching}
    \begin{itemize}
        ...
        \item Delete merged branches
            \begin{shell}
                \alert<4>{\$ git branch -d featureA}\\
                \alert<4>{\$ git branch -d featureB}
            \end{shell}
    \end{itemize}
    \mbox{%
...
\begin{frame}{Retracing Your Steps}
  \begin{enumerate}
    ...
    \item Check out old version
      ...
      \end{shell}
      Note that this does \emph{not change your branch}
    \item Reset your master to the old version
      \begin{shell}
...
\begin{frame}{Fast-Forward}
  \begin{itemize}
    \item Moves your branch label (e.g. ``master'') to the current tip (e.g.
      ``origin/master'') of the branch
      \begin{shell}
        \$ git merge --ff-only origin/branchname
      \end{shell}
    \item Pre-condition is that you did not make any (diverging) commits
  \end{itemize}
\end{frame}

\begin{frame}{Merge Other Branch Into Yours}
  \begin{itemize}
    \item Creates new commit which includes both changes
      \begin{shell}
        \$ git merge origin/master
      \end{shell}
    \item Almost always works, but may result in conflicts if same lines
...
\begin{frame}{If Something Goes Wrong}
  ...
  \begin{enumerate}
    ...
    \item Stop the operation
      ...
      \end{shell}
    \item Undo broken rebases, merges etc:
      \begin{shell}
        \$ git reflog\\
        \$ git checkout HEAD@\{1\}
      \end{shell}
